...
def csv_to_dataset(csv_path):
    ...
    assert (
        ohlcv_histories_normalised.shape[0]
        == next_day_open_values_normalised.shape[0]
...
def multiple_csv_to_dataset(test_set_name):
    ...
    for csv_file_path in list(
        filter(
            lambda x: x.startswith("NYSE"),
            os.listdir(
                r"C:\Users\wclar\iCloudDrive\Desktop\Cardinal Point\Azimuth\NYSE"
            ),
        )
    ):
